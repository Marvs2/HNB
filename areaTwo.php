...
$result = $conn->query("SELECT clientNum FROM areano2 ORDER BY CAST(SUBSTR(clientNum, 2) AS UNSIGNED) DESC LIMIT 1");
if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $lastClientNum = intval(substr($row['clientNum'], 1)) + 1; // Get the numeric part and increment by 1
} else {
    $lastClientNum = 1; // Start from C001 if no records are found
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO `areano2`(`areaTwoId`, `clientNum`, `dpNum`, `firstname`, `lastname`, `middlename`, `graveNo`, `dateofBirth`, `dateofBuried`, `status`, `statCol`, `dpersonId`, `areaNo`, `graveType`, `buriedStatus`, `statusColor`, `dateOfDeath`, `maintenanceStatus`, `maintenanceStatusDate`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $allRecordsInserted = true;

    foreach ($firstnames as $index => $firstname) {
        $clientNum = 'C' . str_pad($lastClientNum++, 3, '0', STR_PAD_LEFT);
        ...
        $stmt->bind_param("isssssssssssssssss", $areaTwoId, $clientNum, $dpNum, $firstname, $lastname, $middlename, $graveNo, $dateofBirth, $dateofBuried, $status, $statCol, $dpersonId, $areaNo, $graveType, $buriedStatus, $statusColor, $dateOfDeath, $maintenanceStatus, $maintenanceStatusDate);
